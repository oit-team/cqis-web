<template>
  <div id="customerList" class="pageCommonStyle">
    ...
    <div class="operateBtn">
      <el-button v-if="btnRole.indexOf(GLOBAL.btnRole.AddBtn) != -1" size="small" icon="el-icon-plus" type="primary" @click="addDataRole">新增数据角色</el-button>
    </div>
    <el-divider/>
    <el-table
      v-loading="loading"
        ...
        <template slot-scope="scope">
          <el-tooltip v-if="btnRole.indexOf(GLOBAL.btnRole.EditBtn) != -1" class="item" effect="dark" content="编辑" placement="top">
            <el-button
              ...
              class="editBtnOnly"
              circle
              @click="editdatarole(scope.row,scope.$index)"
            />
          </el-tooltip>
          <el-tooltip v-if="btnRole.indexOf(GLOBAL.btnRole.DelBtn) != -1" class="item" effect="dark" content="删除" placement="top">
            <el-button
              ...
              class="delBtnOnly"
              circle
              @click="deldataroleItem(scope.row,scope.$index)"
            />
          </el-tooltip>
        </template>
      </el-table-column>
...
export default {
  ...
  created() {
    ...
    this.requestUrl = this.Api.getDataRoleList
  ...
  methods: {
    ...
    deldataroleItem(item, index) {
      this.$confirm('确认删除该角色?', '提示', {
        ...
        _this.$axios.post(_this.Api.delDataRole, jsonParam).then((res) => {
